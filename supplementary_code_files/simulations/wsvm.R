# adapted from svm.R in e1071

# dyn.load("supplementary_code_files/simulations/svmR", .Platform$dynlib.ext)

dyn.load(paste0("supplementary_code_files/simulations/svmR", .Platform$dynlib.ext))
          ...
          na.action = na.omit)
{
    ...
    if (length(weights) != nr)
      stop("x, y and weights don't match.")
    ...
    if (type > 1 && length(class.weights) > 0) {
        class.weights <- NULL
        warning(sQuote("class.weights"), " are set to NULL for regression mode. ",
                "For classification, use a _factor_ for ", sQuote("y"),
...
{
  beta <- colSums(object$SV * c(object$coefs))
  intercept <- -object$rho

  lev <- as.integer(object$levels[object$labels])
  if (lev[1] < 0) {
    beta <- -beta
    intercept <- -intercept
  }
...
function (formula, data = NULL, ..., subset, na.action = na.omit, scale = TRUE)
{
    ...
    ret <- wsvm.default (x, y, scale = scale, ..., na.action = na.action)
    ret$call <- call
